...
{-# LANGUAGE LambdaCase #-}
module SPDX.Document.DocumentCreationInformation where
...
data SPDXCreationInfo = SPDXCreationInfo -- ^One instance is required for each SPDX file produced. It provides the necessary information for forward and backward compatibility for processing tools.
  ...
  , _SPDXCreationInfo_licenseListVersion :: Maybe String -- ^An optional field for creators of the SPDX file to provide the version of the SPDX License List used when the SPDX file was created.
  }
  deriving (Eq, Show)
instance A.FromJSON SPDXCreationInfo where
  parseJSON = A.withObject "SPDXCreationInfo" $ \v ->
    let in SPDXCreationInfo
      <$>   v
      A..:? "comment"
      <*>   v
      A..:  "created"
