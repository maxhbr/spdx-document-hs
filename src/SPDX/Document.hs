...
instance A.FromJSON SPDXDocument where
  parseJSON =
    A.withObject "SPDXDocument" $ \v -> do
      ...
      relationsFromDocumentDescribes <-
        let toRelationship describedId =
         ...
         in fmap
              (map toRelationship . ([] `Maybe.fromMaybe`))
              (v A..:? "documentDescribes")
      SPDXDocument <$> (return documentId) <*> v A..:? "comment" <*>
        v A..: "spdxVersion" <*>
        v A..: "creationInfo" <*>
        v A..: "name" <*>
        v A..: "dataLicense" <*>
        ...
        fmap
          (List.nub .
           (relationsFromDocumentDescribes ++) . ([] `Maybe.fromMaybe`))
          (v A..:? "relationships")
      -- <*> v A..: "revieweds"
      -- <*> v A..: "annotations"
...
getRootsFromDocument (SPDXDocument { _SPDX_SPDXID = documentId
  ...
  (List.nub .
   map _SPDXRelationship_relatedSpdxElement .
   filter
     (\case
...
spdxDocumentToGraph =
  let spdxFileToNode ::
      ...
      spdxRelationToEdge idsToIdxs (r@SPDXRelationship { _SPDXRelationship_comment = _
   ...
   in \(doc@SPDXDocument { _SPDX_name = name
        ...
        let roots = getRootsFromDocument doc
            ...
            relationshipsFromHasFiles =
              concatMap
                (\package ->
                   let pSPDXID = getSPDXID package
